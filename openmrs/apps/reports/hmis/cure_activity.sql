SELECT client_visits.age_group AS 'Age Group',
       ...
       IF(client_visits.patient_id IS NULL, 0, SUM(IF(DATE(client_visits.visit_date) BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE) AND DATE(client_visits.first_visit_date) < DATE(client_visits.visit_date),IF(client_visits.patient_gender = 'M', 1, 0),0))) AS 'Retutning Clients Male',
...
FROM
  (SELECT DISTINCT patient.patient_id AS patient_id,
   ...
   FROM visit
     INNER JOIN patient ON visit.patient_id = patient.patient_id  AND patient.voided = 0 AND visit.voided = 0
